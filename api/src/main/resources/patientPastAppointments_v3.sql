...
SET property_value = 'SELECT
    ...
    (pa.start_date_time AT TIME ZONE CURRENT_SETTING(''timezone'') AT TIME ZONE ''UTC'')
        AS "DASHBOARD_APPOINTMENTS_START_DATE_IN_UTC_KEY",
    (pa.end_date_time AT TIME ZONE CURRENT_SETTING(''timezone'') AT TIME ZONE ''UTC'')
        AS "DASHBOARD_APPOINTMENTS_END_DATE_IN_UTC_KEY"
FROM
